...
function Client(key, options, GenericSocket) {
	...
	self.connection.setTimeout(30000, function() {
		...
		if ((seconds - self._lastTime) >= 5 * 60) {
			self._timedOut = true;
			self.disconnect();
		} else {
			self.raw(['PING', new Date().getTime().toString()]);
		}
	});

	self.queueConnect = _.wrap(self.connection.connect, function(fn, qid) {
		QueuePool.queueJob(self.options.server, qid, self.key, function() {
			fn.call(self.connection);
			self._connecting = true;

			Events.emit([self.key, 'connecting'], {
				nickname: self._nick,
				username: self.options.user,
				server: self.options.server,
				port: self.options.port,
				localPort: self.connection.impl.localPort
			});
...
Client.prototype._ircEvents = {
		...
		for (var argi in message.params) {
			...
			if (match = arg.match(/([A-Z]+)=(.*)/)) {
				...
				if (param == 'NETWORK') {
					...
					if (!this.supported.modes.param) {
						this.supported.modes.param = this.supported.modes.types.a + this.supported.modes.types.b + this.supported.modes.types.c;
					}
	...
	RPL_WHOSPCRPL: function(message) {
		var target = message.params[1].toLowerCase();

		this._addData('channel', target, 'who', [{
			channel: target,
			prefix: message.params[3] + '@' + message.params[4],
			nickname: message.params[6],
			mode: message.params[7],
			extra: message.params[9] || ''
		}]);
		this._addData('channel', target, 'raw', [message.raw]);
	},
	...
	PONG: function(message) {
		this._lastTime = Math.floor((+new Date()) / 1000);
		this._timedOut = false;

		Events.emit([this.key, 'pong'], {
...
Client.prototype.raw = function(data) {
	if (_.isArray(data)) {
		for (var index in data) {
			...
			// ignore and remove none string items. Long standing bug this is.. Fuck nodejs and javascript
...
Client.prototype.privmsg = function(target, message, forcePushBack) {
	...
	var cutAndSend = function (msgStr) {
		var buf = new Buffer(cutOff),
			charLen;

		buf.write(msgStr, 0, 'utf8');
		charLen = buf.toString('utf8').length;
...
Client.prototype._sendRegistered = function(message) {
	...
	self._lastTime = Math.floor((+new Date()) / 1000);
...
Client.prototype._connectEvent = function() {
	if (typeof this.connection.impl.localPort === 'undefined') {
		return false;
	}
	...
	Events.emit([this.key, 'opened'], {
		...
		localPort: this.connection.impl.localPort
	});
	// emit event
};

Client.prototype._closeEvent = function(data) {
	var self = this,
		count = ((this.options.retryCount != null) ? this.options.retryCount : 10) - 1,
		time = this.options.retryWait || 10000,
		reconnect = false;
	...
	if (this._retryCount === count) {
		...
		return;
		// we're done here, emit failed
	} else if (this._retryCount < count || count <= 0) {
		if (this._requestedDisconnect && !this._timedOut) {
			reconnect = false;
			// bail now.
		} else {
			reconnect = true;
